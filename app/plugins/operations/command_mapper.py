...
class OperationCommand(Command):
    ...
    def execute(self, *args, **kwargs):
        ...
        """
        self.logger.info(f"Executing operation command: {self.operation_name} with args: {args}")
        ...
        self.logger.info(f"Operation {self.operation_name} result: {result}")
        return result
...
def register_operation_commands(command_handler):
    ...
    for op_name in operations:
        ...
        logging.info(f"Registered command for operation: {op_name}")
    
    logging.info(f"Successfully registered {len(operations)} operation commands") 